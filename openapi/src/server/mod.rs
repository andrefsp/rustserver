            ...
            &hyper::Method::POST if path.matched(paths::ID_USER) => {
                {
                    ...
                    if let Scopes::Some(ref scopes) = authorization.scopes {
                        let required_scopes: std::collections::BTreeSet<String> = vec![
                            "write:users".to_string(), // modify pets in your account
                            "read:users".to_string(), // read your pets
                        ].into_iter().collect();
                ...
                match result {
                            Ok(body) => {
                                ...
                                let param_create_user_request: Option<models::CreateUserRequest> = if !body.is_empty() {
                                    let deserializer = &mut serde_json::Deserializer::from_slice(&*body);
                                    match serde_ignored::deserialize(deserializer, |path| {
                                            warn!("Ignoring unknown field in body: {}", path);
                                            unused_elements.push(path.to_string());
                                    }) {
                                        Ok(param_create_user_request) => param_create_user_request,
                                        Err(_) => None,
                                    }
                                } else {
                                    None
                            ...
                            Err(e) => Ok(Response::builder()
                                                ...
                                                .body(Body::from(format!("Couldn't read body parameter CreateUserRequest: {}", e)))
                                                .expect("Unable to create Bad Request response due to unable to read body parameter CreateUserRequest")),
                        }
            },
